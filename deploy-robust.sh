...
echo "   ${GREEN}# SNKRDUNK scan every 6 hours${NC}"
echo "   ${GREEN}0 6,12,18,0 * * * cd $APP_DIR && source $VENV_DIR/bin/activate && python -c \"import requests; requests.post('http://localhost:8000/api/v1/snkrdunk/fetch', json={'pages': [1,2,3,4,5], 'force_refresh': True})\" >> ~/logs/snkrdunk_cron.log 2>&1${NC}"
echo ""
echo "   ${GREEN}# Competitor scans every 6 hours${NC}"
echo "   ${GREEN}0 6,12,18,0 * * * cd $APP_DIR && source $VENV_DIR/bin/activate && python -c \"import requests; requests.post('http://localhost:8000/api/v1/scrape/all-competitors')\" >> ~/logs/competitor_cron.log 2>&1${NC}"
echo ""
